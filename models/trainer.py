...
class CDTrainer():
    ...
    # 加热力图
    # def _visualize_pred(self):
    #     pred_probabilities = torch.softmax(self.G_final_pred, dim=1)
    #     return pred_probabilities.cpu().numpy()
    ...
    def _backward_G(self):
        ...
        #print("gt.size",gt.shape) 与loss里面的target一样
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            for self.batch_id, batch in enumerate(self.dataloaders['val'], 0):
                ...
                # Visualize and save the heatmap images
                pred_probabilities = self._visualize_pred()
                for i in range(self.batch_size):
                    heatmap = pred_probabilities[i, 0]  # Assuming the class you want to visualize is class 1
                    heatmap = heatmap.cpu().numpy()
                    sns.heatmap(heatmap, cmap="coolwarm", cbar=False)
                    plt.title(f"Heatmap for Batch {self.batch_id}, Image {i}")
                    plt.savefig(f"/media/dsk2/zhongyu.zhang/project/ChangeFormer/vis/CD_SNUNet_LEVIR/heatmap_batch{self.batch_id}_image{i}.png")

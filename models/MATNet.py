...
class LKA(nn.Module):
    ...
    def forward(self, x):
        u = x.clone()    #[6, 128, 32, 32]
        attn = self.conv0(x)
        attn = self.conv_spatial(attn)
        attn = self.conv1(attn)   #[6, 128, 32, 32]

        return u * attn
class STC(nn.Module):
    def  __init__(self, in_channels):
        super().__init__()
        self.proj_1 = nn.Conv2d(in_channels, out_channels=in_channels, kernel_size=1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = LKA(in_channels)
        self.proj_2 = nn.Conv2d(in_channels, out_channels=in_channels, kernel_size=1)
...
class SCA(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        ...
        # CBR block
        self.conv5a = nn.Sequential(nn.Conv2d(in_channels, out_channels, 3, padding=1, bias=False),
    ...
    def forward(self, x1,x2):
        ...
        #print(x.shape)
        feat1 = self.conv5a(x)    #[6,128,32,32]
        sa_feat = self.sa(feat1)  # [6, 128, 32, 32]
        sa_conv = self.conv51(sa_feat)  # (B, 128, H, W)
        sa_output = self.conv6(sa_conv)  # (B, 128, H, W)

        return sa_output  # (B, 128, H, W)

# class Diff_Sum(nn.Module):
...
class MAM(nn.Module):
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        # self.diff_sum1=Diff_Sum(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim)
        # self.diff_sum2=Diff_Sum(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim)
        # self.diff_sum3=Diff_Sum(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim)
        # self.diff_sum4=Diff_Sum(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim)

        self.sca1=SCA(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim,norm_layer=norm_layer)
        self.sca2=SCA(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim,norm_layer=norm_layer)
    ...
    def forward(self, inputs1, inputs2):
        ...
        outputs = []
        f1_4 = self.linear_c4(c4_1).permute(0,2,1).reshape(n, -1, c4_1.shape[2], c4_1.shape[3])
        f2_4 = self.linear_c4(c4_2).permute(0,2,1).reshape(n, -1, c4_2.shape[2], c4_2.shape[3])
        f1_3 = self.linear_c3(c3_1).permute(0,2,1).reshape(n, -1, c3_1.shape[2], c3_1.shape[3])
        f2_3 = self.linear_c3(c3_2).permute(0,2,1).reshape(n, -1, c3_2.shape[2], c3_2.shape[3])
        f1_2 = self.linear_c2(c2_1).permute(0,2,1).reshape(n, -1, c2_1.shape[2], c2_1.shape[3])
        f2_2 = self.linear_c2(c2_2).permute(0,2,1).reshape(n, -1, c2_2.shape[2], c2_2.shape[3])
        f1_1 = self.linear_c1(c1_1).permute(0,2,1).reshape(n, -1, c1_1.shape[2], c1_1.shape[3])
...
class matnet(nn.Module):
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.decoder=MAM(input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False, 
                    in_channels = self.embed_dims, embedding_dim= self.embedding_dim, output_nc=output_nc, 
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16],norm_layer=nn.BatchNorm2d)
    def forward(self, x1, x2):
